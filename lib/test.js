...
const newBlockchain = ({protocol = 'ws:'} = {}) =>
  exec(`
...
const deterministic = generic.function()

deterministic.method(_, R.identity)

// Override the changing 'tx_hash' with a constant value.
deterministic.method([{
  ...
  ['result', 'tx_hash'],
  'CB475DEB22EFEE014AE8968CBB2AF7597349F7A9'
  )
)

// Make the output of Eris DB deterministic to ease testing.
const makeDeterministic = (transport) =>
  (input) =>
    I.map(deterministic, transport(input))
...
const VectorRecord = () => {
  ...
  const run = (callback, thisArg) =>
    Promise.try(callback.bind(thisArg, {db, validator: vectors.validator}))
      .then(() => {
        vector.push(Promise.resolve({done: true}))
        return I.to(Array, vector)
      })
  ...
  return Object.assign(Vector(), {
    before: (newDirname, options, callback) =>
      function () {
        ...
        vectors.validator = require(
          untildify(`~/.eris/chains/${name}/priv_validator.json`)
        )

        return newBlockchain(options).then((urlString) => {
          transport = makeDeterministic(jsonRpc.transport(url.parse(urlString)))
          const {memoized, vector: newVector} = testVector.memoize(transport)
          vector = newVector
          db = ErisDb.createInstance(memoized)

          if (callback) {
            return run(callback, this).then((vectorArray) => {
...
const VectorPlay = () => {
  ...
  const run = (recording, callback, thisArg) => {
    ...
    return callback.call(thisArg, {db, validator: vectors.validator})
  }
  ...
  return Object.assign(Vector(), {
    before: (dirname, options, callback) =>
      function () {
        ...
        const transport = testVector.mockFunction(vector).mock
        db = ErisDb.createInstance(transport)
