    ...
    if (initDir) {
      G.forEach((file, contents) => {
        fs.writeFileSync(path.join(
          os.homedir(),
          `.monax/chains/${name}/${name}_full_000/${file}`
        ), JSON.stringify(contents, null, 2))
      }, initDir)
    }

    return exec(`monax chains start \
      --init-dir ~/.monax/chains/${name}/${name}_full_000 \
      --publish ${name}
        ...
        return startBlockchain(blockchainName(dirname), options)
          .then((urlString) => {
            const initDirPath = path.join(
              os.homedir(),
              `.monax/chains/${blockchainName(dirname)}/` +
                `${blockchainName(dirname)}_full_000`
            )
