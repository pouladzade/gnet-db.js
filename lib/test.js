...
const startBlockchain = (name, {protocol = 'ws:'} = {}) => {
  return exec(`
    ...
    before: (newDirname, options, callback) =>
      function () {
        ...
        return startBlockchain(blockChainname(dirname), options)
          .then((urlString) => {
            vectors.validator = require(path.join(
              os.homedir(),
              `.eris/chains/${blockChainname(dirname)}/priv_validator.json`
            ))

            transport = makeDeterministic(jsonRpc.transport(url.parse(
              urlString))
            )
...
const VectorPlay = () => {
  ...
  const run = (recording, callback, thisArg) =>
    new Promise((resolve, reject) => {
      ...
      return Promise.try(() =>
        callback.call(thisArg, {db, validator: vectors.validator})
      ).then(resolve, reject)
    })
